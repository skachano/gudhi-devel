...
Mesh_medit build_mesh_from_cell_complex(const Cell_complex& cell_complex,
  ...
  Mesh_medit output;
  std::map<Hasse_cell*, std::size_t> vi_map; // one for vertices
  ...
  for (const auto& cp_pair: cell_complex.cell_point_map()) {
    ...
    // std::cout << index << ": " << cell_complex.cell_simplex_map().at(cp_pair.first) << "\n";
...
#endif
  Configuration b2_configuration = Configuration(true, true, true, 6, 16, 17);
  populate_mesh(output, cell_complex.boundary1_simplex_cell_maps(), b_configuration, amb_d, vi_map);  
  populate_mesh(output, cell_complex.boundary2_simplex_cell_maps(), b2_configuration, amb_d, vi_map);  
  populate_mesh(output, cell_complex.corner_simplex_cell_maps(), b_configuration, amb_d, vi_map);  
#ifdef GUDHI_COX_OUTPUT_TO_HTML
