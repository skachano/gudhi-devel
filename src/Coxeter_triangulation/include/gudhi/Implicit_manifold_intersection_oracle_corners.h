...
template<class Function_,
	 class Domain_function1_ = Constant_function,
	 class Domain_function2_ = Constant_function>
class Implicit_manifold_intersection_oracle {
  ...
  template <class Simplex_handle, 
	    class Triangulation>
  Eigen::VectorXd compute_boundary1_lambda(const Simplex_handle& simplex,
					  const Triangulation& triangulation) const {
    std::size_t cod_d = this->cod_d();
    ...
    for (auto v: simplex.vertex_range()) {
      ...
      for (std::size_t i = 1; i < cod_d + 1; ++i)
	matrix(i, j) = v_coords(i-1);
      Eigen::VectorXd bv_coords = domain_fun1_(triangulation.cartesian_coordinates(v));
  ...
  template <class Simplex_handle, 
	    class Triangulation>
  Eigen::VectorXd compute_boundary2_lambda(const Simplex_handle& simplex,
					   const Triangulation& triangulation) const {
    std::size_t cod_d = this->cod_d();
    Eigen::MatrixXd matrix(cod_d + 2, cod_d + 2);
    for (std::size_t i = 0; i < cod_d + 2; ++i)
    ...
    for (auto v: simplex.vertex_range()) {
      ...
      for (std::size_t i = 1; i < cod_d + 1; ++i)
	matrix(i, j) = v_coords(i-1);
      Eigen::VectorXd bv_coords = domain_fun2_(triangulation.cartesian_coordinates(v));
      matrix(cod_d + 1, j) = bv_coords(0);
      j++;
    }
    Eigen::VectorXd z(cod_d + 2);
  ...
  template <class Simplex_handle, 
	    class Triangulation>
  Eigen::VectorXd compute_corner_lambda(const Simplex_handle& simplex,
					const Triangulation& triangulation) const {
    std::size_t cod_d = this->cod_d();
    Eigen::MatrixXd matrix(cod_d + 3, cod_d + 3);
    for (std::size_t i = 0; i < cod_d + 3; ++i)
      matrix(0, i) = 1;
    std::size_t j = 0;
    for (auto v: simplex.vertex_range()) {
      Eigen::VectorXd v_coords = fun_(triangulation.cartesian_coordinates(v));
      for (std::size_t i = 1; i < cod_d + 1; ++i)
	matrix(i, j) = v_coords(i-1);
      Eigen::VectorXd bv_coords1 = domain_fun1_(triangulation.cartesian_coordinates(v));
      matrix(cod_d + 1, j) = bv_coords1(0);
      Eigen::VectorXd bv_coords2 = domain_fun2_(triangulation.cartesian_coordinates(v));
      matrix(cod_d + 2, j) = bv_coords2(0);
      j++;
    }
    Eigen::VectorXd z(cod_d + 3);
    z(0) = 1;
    for (std::size_t i = 1; i < cod_d + 3; ++i)
      z(i) = 0;
    Eigen::VectorXd lambda = matrix.colPivHouseholderQr().solve(z);
    return lambda;
  }
...
public:
  ...
  template <class Simplex_handle,
	    class Triangulation>
  Query_result<Simplex_handle> intersects_boundary1(const Simplex_handle& simplex,
						    const Triangulation& triangulation) const {
    Eigen::VectorXd lambda = compute_boundary1_lambda(simplex, triangulation);
    return intersection_result(lambda, simplex, triangulation);
  }

  template <class Simplex_handle,
	    class Triangulation>
  Query_result<Simplex_handle> intersects_boundary2(const Simplex_handle& simplex,
						    const Triangulation& triangulation) const {
    Eigen::VectorXd lambda = compute_boundary2_lambda(simplex, triangulation);
    return intersection_result(lambda, simplex, triangulation);
  }

  template <class Simplex_handle,
	    class Triangulation>
  Query_result<Simplex_handle> intersects_corner(const Simplex_handle& simplex,
						 const Triangulation& triangulation) const {
    Eigen::VectorXd lambda = compute_corner_lambda(simplex, triangulation);
    return intersection_result(lambda, simplex, triangulation);
  }
  ...
  bool lies_in_domain1(const Eigen::VectorXd& p,
		       const Triangulation& triangulation) const {
    Eigen::VectorXd pl_p = make_pl_approximation(domain_fun1_, triangulation)(p);
    return pl_p(0) < 0;
  }

  template <class Triangulation>
  bool lies_in_domain2(const Eigen::VectorXd& p,
		       const Triangulation& triangulation) const {
    Eigen::VectorXd pl_p = make_pl_approximation(domain_fun2_, triangulation)(p);
    return pl_p(0) < 0;
  }
  ...
  Implicit_manifold_intersection_oracle(const Function_& function,
					const Domain_function1_& domain_function1,
					const Domain_function2_& domain_function2)
    : fun_(function), domain_fun1_(domain_function1), domain_fun2_(domain_function2) {}
   ...
   */
  Implicit_manifold_intersection_oracle(const Function_& function,
					const Domain_function1_& domain_function)
    : fun_(function), domain_fun1_(domain_function),
      domain_fun2_(function.amb_d(), 1, Eigen::VectorXd::Constant(1,-1)) {}
  ...
  Implicit_manifold_intersection_oracle(const Function_& function)
    : fun_(function),
      domain_fun1_(function.amb_d(), 1, Eigen::VectorXd::Constant(1,-1)),
      domain_fun2_(function.amb_d(), 1, Eigen::VectorXd::Constant(1,-1)) {}
  
private:
  Function_ fun_;
  Domain_function1_ domain_fun1_;
  Domain_function2_ domain_fun2_;
};

template<class Function_,
	 class Domain_function1_,
	 class Domain_function2_>
Implicit_manifold_intersection_oracle<Function_, Domain_function1_, Domain_function2_>
make_oracle(const Function_& function,
	    const Domain_function1_& domain_function1,
	    const Domain_function2_& domain_function2){
  return Implicit_manifold_intersection_oracle<Function_,
					       Domain_function1_,
					       Domain_function2_>(function,
								  domain_function1,
								  domain_function2);
}
